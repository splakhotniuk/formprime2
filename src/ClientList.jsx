...
import { PrecizeDeleteWindow } from './PrecizeDeleteWindow'

export const ClientList = (props) => {
    console.log('Пропсы List', props);
  const [data, setData] = useState([]);
  const [expandedRows, setExpandedRows] = useState(null);
  const [isPrecizeDeleteWindow, setIsPrecizeDeleteWindow] = useState(false);
  const [currentClient, setCurrentClient] = useState(null);
  const dataContext = useContext(DataContext);

  const fetchData = () => {
        if ( !isPrecizeDeleteWindow ) {
  ...
  useEffect(fetchData, [isPrecizeDeleteWindow])
  ...
  const rowExpansionTemplate = (data) => {
      return (
        <div>
            ...
            <Button label='Видалити дані' className='p-button-danger' onClick={() => {setIsPrecizeDeleteWindow(true); setCurrentClient(data)}}/>
           
        </div>
    );
  }

  const renderDialogMessage = (currentClient) => {
    if ( isPrecizeDeleteWindow ) {
        return (
            <PrecizeDeleteWindow setIsPrecizeDeleteWindow={setIsPrecizeDeleteWindow} currentClient={currentClient}/>
        )
    } 
}
    ...
    return (
        <div>
            {renderDialogMessage(currentClient)}
            ...
            <div>
                <Link to='/'><Button label='<-- На стартову сторінку'/></Link>
            </div>
        </div>
    );
};
